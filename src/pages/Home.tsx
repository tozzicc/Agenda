...
export function Home() {
    ...
    const optionButton = (title: string, subtitle: string | null, onClick: () => void) => (
        ...
        <main className="flex-grow flex flex-col items-center justify-center pb-12 px-4 sm:px-6">
            ...
            <div className="w-full max-w-3xl rounded-3xl border border-gray-100 bg-white p-6 shadow-xl md:p-8">
                {step === 'success' && bookingData && selectedDate && selectedTime && selectedService && selectedProfessional ?
                    <Confirmation selectedDate={selectedDate} selectedTime={selectedTime} bookingData={bookingData} serviceName={selectedService.name} professionalName={selectedProfessional.name} onReset={reset} /> : <>
                    <p className="mb-6 text-sm font-medium text-indigo-600">Serviço → Profissional → Data → Horário → Dados → Confirmação</p>
